...
+ (instancetype)wmf_placesActivityWithURL:(NSURL *)activityURL {
    ...
    // NOTE: Not using kCLLocationCoordinate2DInvalid because setting valid latitude without valid longitude results in improperly-valid coordinate
    CLLocationCoordinate2D searchCoordinate = CLLocationCoordinate2DMake(DBL_MAX, DBL_MAX); // DBL_MAX instead of 0 so resulting coordinate is invalid by default
    ...
    for (NSURLQueryItem *item in components.queryItems) {
        if ([item.name isEqualToString:@"WMFArticleURL"]) {
            ...
            NSNumber *parsedLatitude = [self wmf_coordinatePartForString:item.value];
            
            if (parsedLatitude) {
                searchCoordinate.latitude = parsedLatitude.doubleValue;
            }
        }
        else if ([item.name isEqualToString:@"longitude"]) {
            NSNumber *parsedLongitude = [self wmf_coordinatePartForString:item.value];
            
            if (parsedLongitude) {
                searchCoordinate.longitude = parsedLongitude.doubleValue;
            }
        }
    }
    ...
    if (CLLocationCoordinate2DIsValid(searchCoordinate)) {
        activity.mapItem = [[MKMapItem alloc] initWithPlacemark:[[MKPlacemark alloc] initWithCoordinate:searchCoordinate]];
        activity.mapItem.name = activity.title = placeName;
    }
...
+ (nullable NSNumber *)wmf_coordinatePartForString:(nonnull NSString *)base {
    
    // Based on NSNumber(Formatter) to allow for distinguishing between nil and a legitimate 0
    
    static NSNumberFormatter *coordinateParser = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coordinateParser = [NSNumberFormatter new];
        coordinateParser.numberStyle = NSNumberFormatterDecimalStyle;
        coordinateParser.generatesDecimalNumbers = YES;
        coordinateParser.locale = [NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];
    });
    
    return [coordinateParser numberFromString:base];
}
